...
void Paint_3phase(char a, char b, char c)
{
    ...
    unsigned int Length = WorkParameters[1];
    signed int mid = ((Cardio_Y_Max - Cardio_Y_Min)>>1) + Cardio_Y_Min; 
    unsigned int temp;
    char i;
    ...
    // очищаем за собой столбец пикселей
    SSD1963_DrawFillRect(Cardio_X, Cardio_X + Length, Cardio_Y_Min, Cardio_Y_Max + 1, BLACK);
     
    //Линия нуля 
    SSD1963_DrawFastLine(Cardio_X, Cardio_X + Length, mid, mid, DGRAY);
    SSD1963_DrawFastLine(Cardio_X, Cardio_X + Length, mid+50, mid+50, DGRAY);
    SSD1963_DrawFastLine(Cardio_X, Cardio_X + Length, mid-50, mid-50, DGRAY);
    ...
    for(i=0; i<3; i++)
    {
        ...
        Value[i] += mid; 
        if(Value[i] >= Cardio_Y_Max) Value[i] = Cardio_Y_Max - 1;
        if(Value[i] < Cardio_Y_Min) Value[i] = Cardio_Y_Min; 

        // пишем точку в столбце
        SSD1963_DrawLine(Cardio_X, Cardio_X + Length, ValueLast[i], Value[i], Color[i], 2);  
        ValueLast[i] = Value[i]; // определяем предыдущую точку кардиограммы
    }
...
void Paint_2phase(char a, char b)
{
    ...
    //Линия нуля 
    //SSD1963_DrawFastLine(Cardio_X, Cardio_X + Length, mid, mid, DGRAY);
    SSD1963_DrawFastLine(Cardio_X, Cardio_X + Length, mid+40, mid+40, DGRAY);
    SSD1963_DrawFastLine(Cardio_X, Cardio_X + Length, mid-40, mid-40, DGRAY);
    ...
    for(i=0; i<2; i++)
    {
        ...
        if(i==0) Value[i] += mid+40; 
        if(i==1) Value[i] += mid-40; 
        if(Value[i] >= Cardio_Y_Max) Value[i] = Cardio_Y_Max - 1;
        if(Value[i] < Cardio_Y_Min) Value[i] = Cardio_Y_Min; 
...
while (1)
{
        ...
        switch(State)
        {
          case 0 : // основной рабочий режим
          {
            ...
            //ADC0 = read_adc(0);
            //if (ADC0 > WorkParameters[0]) ADC0 = WorkParameters[0];
           // putchar(Address_Slave);
           // if (rx_counter0 > 0)
           // {
              //  ADC0 = ((getchar() << 8) + getchar());
            //}
           ...
           // Cardio_Divider = WorkParameters[0] / (Cardio_Y_Max - Cardio_Y_Min);
            ...
            if (TSC2046_GetCoordinates() > 0)
            {
              ...
              switch (Button_Pressed)
              {
                ...
                break;
                case 2 :
                if(multiplier<200)multiplier+=10;
